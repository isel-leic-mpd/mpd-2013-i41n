...
public class JdbcSuppliersMapper extends JdbcAbstractMapper<Integer, Supplier>{
	...
	private final JdbcConverter<Supplier> conv= new JdbcConverter<Supplier>() {
		...
		public Supplier convert(ResultSet rs) throws SQLException {
			final int supplierID = rs.getInt(1);
			return new Supplier(
					supplierID, 
					rs.getString(2), 
					rs.getString(3), 
					...
					rs.getString(5), 
					rs.getString(6), 
					new Iterable<Product>() {
						@Override
						public Iterator<Product> iterator() {
							return prodsMapper.where("SupplierId = ?", JdbcBinder.BindInt, supplierID).iterator();
						}
					});
		}
	};
	...
	@Override
	protected Integer getKeyFromValue(Supplier value) {
		return value.getId();
	}
